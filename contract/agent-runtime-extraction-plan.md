...
- 纯函数工具（如 `_subprocess_compat.windows_hide_flags`、`cfg_get`、`timefmt.coerce_epoch`）**不设 Port**，Standalone 构建时整模块随行复制（§8.2）。
...
| 桶 | 处理方式 | 典型符号 |
|---|---|---|
| ① 纯函数工具 | 随行复制，不设 Port | `_subprocess_compat.*`（39）、`config.cfg_get`、`timefmt`、`sizefmt`、`route_identity.normalize_route_base_url` |
...
| `HostProcessPort` | `gateway.status` 17 | `pid_exists` / `process_start_time` / `running_pid` | `psutil` 实现 |
...
| 随行（纯函数） | `hermes_cli._subprocess_compat`（39） | `windows_hide_flags`（21/6）、`harden_git_argv`、`noninteractive_git_env`、`IS_WINDOWS`、`kill_process_tree` |
