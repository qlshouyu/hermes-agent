...
| **EventSinkPort** | 宿主实现，运行时调用 | `emit(event)` | 接收单向事件：流式输出、思考、工具开始 / 进度 / 完成、状态、通知、表情反应；对应上游 16 个通知类回调 |
| **InteractionPort** | 宿主实现，运行时调用 | `clarify`、`approve`、`read_terminal`、`capture_secret` | 需要人参与并拿回结果的交互；对应上游 5 个交互类构造回调，以及 `set_approval_callback` 等进程级回调（按当前会话 ID 分发） |
